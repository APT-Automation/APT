...
public class APT_CreateAccessCoreDevice_ManageNetworkHelper extends DriverHelper {
	...
	XMLReader xml = new XMLReader("src\\com\\colt\\qa\\pagerepository\\ManageNetwork_createAccessCOreDevice.xml");
	...
	try {	
		...
		if(managementaddress.equalsIgnoreCase("null")) {
			ExtentTestManager.getTest().log (LogStatus.PASS, "'Fetch Interface' column do not display, if no values passed for 'Management Address'");
			Log.info("'Fetch Interface' column do not display, if no values passed for 'Management Address'");
		}else {
...
//			compareText(application, "Snmpro value", "statuspage_snmprovalue", snmpro, xml);
		...
		//verify vistamart device value
			GetText(application, "VistaMart Device", "synchronization_vistamartValue_noManageAddress");
			//verify vistamart device date time
			try {
				String GMTValue;
				String VistaMartDevicevalue= getwebelement(xml.getlocator("//locators/" + application + "/vistamartDEvice_dateTime_noManageAddress")).getText();
	...
	public void manageNetworkPageUpdated(String application, String devicename, String Inservice_status, String Inmaintenance_status, String vendormodel, String managementaddress, 
		String snmpro, String country, String interfacename) throws InterruptedException, DocumentException {
