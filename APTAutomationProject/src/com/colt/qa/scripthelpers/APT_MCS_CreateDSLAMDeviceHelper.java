...
public class APT_MCS_CreateDSLAMDeviceHelper extends DriverHelper{

	public APT_MCS_CreateDSLAMDeviceHelper(WebDriver dr) {
		super(dr);
    ...
    //Verify Mandatory fields
        ...
        Thread.sleep(2000);
			...
			String expectedMessage="Appending .dslam.colt.net to device name";
		try {	
			alertPopup=getwebelement(xml.getlocator("//locators/" + application + "/DSLAMdevice_alertmessage")).isDisplayed();
			if(alertPopup) {
				ExtentTestManager.getTest().log(LogStatus.PASS, "An alert popup message dipslays, when 'device name' selected as 'DSLAM device'");
				Log.info("An alert popup message dipslays, when 'device name' selected as 'DSLAM device'");
				
				alertMessage=getwebelement(xml.getlocator("//locators/" + application + "/DSLAMdevice_alertmessage")).getText();
				if(expectedMessage.equalsIgnoreCase("alertMessage")) {
					ExtentTestManager.getTest().log(LogStatus.PASS, "Alert message is displaying as: "+alertMessage);
					Log.info( "Alert message is displaying as: "+alertMessage);
					
					click_commonMethod(application, "OK", "DSLAMdevice_popupOkbutton", xml);
				}
				else if(expectedMessage.contains(alertMessage)) {
					ExtentTestManager.getTest().log(LogStatus.PASS, "Alert message is displaying as: "+alertMessage);
					Log.info( "Alert message is displaying as: "+alertMessage);
					...
					ExtentTestManager.getTest().log(LogStatus.FAIL, "Alert Message is mismatching. It is displaying as: "+alertMessage);
					Log.info("Alert Message is mismatching. It is displaying as: "+alertMessage);
				...
				ExtentTestManager.getTest().log(LogStatus.FAIL, "An alert popup message is not dipslaying, when 'device name' selected as 'DSLAM device'");
				Log.info("An alert popup message is not dipslaying, when 'device name' selected as 'DSLAM device'");
			}
		}catch(Exception e) {
			e.printStackTrace();
			ExtentTestManager.getTest().log(LogStatus.FAIL, "An alert popup message is not dipslaying, when 'device name' selected as 'DSLAM device'");
	...
	//Fetch Device name
		String devicename=getwebelement(xml.getlocator("//locators/" + application + "/nametextfield")).getAttribute("value");
		if(devicename.endsWith("dslam.colt.net")) {
			ExtentTestManager.getTest().log(LogStatus.PASS, "device name is displaying as: "+devicename);
			Log.info("device name is displaying as: "+devicename);
			
			deviceName=devicename;
		}else {
			ExtentTestManager.getTest().log(LogStatus.FAIL, "device name is displaying as: "+devicename);
			Log.info("device name is displaying as: "+devicename);
    ...
    //Device Name	
    	compareText_InViewPage(application, "Name", deviceName, xml);
    ...
    //Device Name
    	...
    	//Device Type
     		Select sel = new Select(getwebelement(xml.getlocator("//locators/" + application + "/devicetypeinput")));
	   		...
	   		assertEquals(devicetype.getText(), editDeviceType);
	   		if(devicetype.getText().equalsIgnoreCase("DSLAM Device")) {
